...
fun AllSeasonalMedia(
    ...
    val seasonalViewModel : SeasonalViewModel = koinViewModel<SeasonalViewModel>(
        parameters = {
            parametersOf(season, type, currentYear)
        }
    )

    val seasonalMedias = seasonalViewModel.seasonalMedias.collectAsLazyPagingItems()
    ...
    Scaffold(
        ...
        Box(
            ...
            if(seasonalMedias.loadState.refresh == LoadState.Loading) {
                LoadingMediaList()
            } else {
                ...
                if(isList) {
                    ...
                    LazyColumn(modifier = Modifier.padding(horizontal = 12.dp)) {
                        items(seasonalMedias.itemCount) {
                                index ->
                            seasonalMedias[index]?.let {
                                AnimeListTile(
                                    id = it.id ?: 0,
