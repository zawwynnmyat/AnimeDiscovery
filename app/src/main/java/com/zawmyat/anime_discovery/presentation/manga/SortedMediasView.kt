...
@Composable
fun SortedMediasView(
    sort: String,
    type: String,
    navController : NavHostController
) {

    val sortMediaViewModel : SortMediaViewModel = koinViewModel<SortMediaViewModel>(
        parameters = {
            parametersOf(sort, type)
        }
    )

    val sortedMedias = sortMediaViewModel.sortedMedias.collectAsLazyPagingItems()
    ...
    Scaffold(
        topBar = {
            TrendingAppBar(
                title = "${if(sort == "Top 100") "Highest Rated" else sort} ${type}s",
                isList = isList,
                onNavBackClick = {
        ...
        Box(
            ...
            if(sortedMedias.loadState.refresh == LoadState.Loading) {
                ...
                if(isList) {
                    ...
                    LazyColumn(modifier = Modifier.padding(horizontal = 12.dp)) {
                        items(sortedMedias.itemCount) {
                                index ->
                            sortedMedias[index]?.let {
                                AnimeListTile(
                                    id = it.id ?: 0,
