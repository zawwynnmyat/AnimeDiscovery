...
@Composable
fun StatusFilteredMediaView(
    status: String,
    type: String,
    navController : NavHostController
) {

    val statusFilteredMediasViewModel : StatusFilteredMediaViewModel = koinViewModel<StatusFilteredMediaViewModel>(
        parameters = {
            parametersOf(status, type)
        }
    )

    val statusFilteredMedias = statusFilteredMediasViewModel.statusFilteredMedias.collectAsLazyPagingItems()
    ...
    Scaffold(
        ...
        Box(
            ...
            if(statusFilteredMedias.loadState.refresh == LoadState.Loading) {
                LoadingMediaList()
            } else {
                if(isList) {

                    LazyColumn(modifier = Modifier.padding(horizontal = 12.dp)) {
                        items(statusFilteredMedias.itemCount) {
                                index ->
                            statusFilteredMedias[index]?.let {
                                AnimeListTile(
                                    id = it.id ?: 0,
