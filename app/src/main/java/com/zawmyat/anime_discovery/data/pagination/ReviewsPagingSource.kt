...
class ReviewsPagingSource(
...
) : PagingSource<Int, ReviewsQuery.Review>() {

    override fun getRefreshKey(state: PagingState<Int, ReviewsQuery.Review>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ReviewsQuery.Review> {
        return try {
            val page = params.key ?: 1
            val response = mainRepository.fetchAllReviews(page = page, perPage = 10).first()
