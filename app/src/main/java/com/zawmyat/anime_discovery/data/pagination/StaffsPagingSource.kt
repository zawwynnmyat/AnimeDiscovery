...
class StaffsPagingSource(
    val mainRepository: MainRepository
) : PagingSource<Int, StaffsQuery.Staff>() {

    override fun getRefreshKey(state: PagingState<Int, StaffsQuery.Staff>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, StaffsQuery.Staff> {
        return try {
            val page = params.key ?: 1
            val response = mainRepository.fetchAllStaffs(page = page, perPage = 10).first()
