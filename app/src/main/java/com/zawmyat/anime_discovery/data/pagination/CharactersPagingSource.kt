...
class CharactersPagingSource(
...
) : PagingSource<Int, CharactersQuery.Character>() {

    override fun getRefreshKey(state: PagingState<Int, CharactersQuery.Character>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, CharactersQuery.Character> {
        return try {
            val page = params.key ?: 1
            val response = mainRepository.fetchAllCharacters(page = page, perPage = 10).first()
